...
class AgentRunner:
    ...
    async def run_with_stream(
        ...
        """
        执行 Agent 循环，最终回复使用流式输出

        Args:
            messages: 初始消息列表（包含 system prompt）
            progress_callback: 进度回调，用于显示中间过程
            stream_callback: 流式输出回调，逐字输出最终回复
        ...
        while iteration < self.max_iterations:
            ...
            # 如果没有 tool_calls，用流式输出
            if not response.tool_calls:
                if stream_callback and response.content:
                    _progress("[cyan]输出结果中...[/cyan]")
                    for delta in response.content:
    ...
    async def run_full_stream(
        ...
        """
        执行 Agent 循环，全程流式输出（流式工具调用结果 + 最终回复）
            ...
            if not response.tool_calls:
                if stream_callback and response.content:
                    ...
                    for delta in response.content:
                        full_content += delta
                        stream_callback(delta)
                return LLMResponse(content=full_content or response.content)
