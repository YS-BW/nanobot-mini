...
当前 compact 分两层：
 - 第一层：裁剪当前 thread 的消息窗口，生成 `summary.jsonl`
...
class CompactService:
    ...
    async def run_if_needed(self, force: bool = False) -> bool:
        """按策略决定是否执行 compact。"""
        if not force and not self.policy.should_compact(self.thread_store.estimate_tokens()):
            return False
        await self._compact()
        return True
    ...
    async def _compact(self) -> None:
        ...
        if summary:
            self.thread_store.append_summary(summary.strip())

        if self.policy.should_rollup_summary(self.thread_store.get_summary_count()):
            await self._compact_round2()

    def _get_trimmed(self) -> list[dict]:
        """取出本轮要从 session 中裁掉的消息。"""
        if len(self.thread_store.messages) > self.policy.keep_count:
            trimmed = self.thread_store.messages[:-self.policy.keep_count]
            self.thread_store.messages = self.thread_store.messages[-self.policy.keep_count :]
        else:
            trimmed = self.thread_store.messages.copy()
            self.thread_store.messages = []
...
"""
    ...
    async def _compact_round2(self) -> None:
        """把累计摘要整合成长期记忆。"""
        summary_entries = self.thread_store.get_summary_content()
        if not summary_entries:
            return

        memory_path = self.thread_store.memory_path
        existing_memory = ""
...
"""
        try:
            ...
            if content.strip() and memory_path:
                ...
                self.thread_store.clear_summary()
        except Exception:
            pass
